...
size_of.cancelled_shares = 4

-- Display: Cancelled Shares
display.cancelled_shares = function(value)
  return "Cancelled Shares: "..value
end

-- Dissect: Cancelled Shares
dissect.cancelled_shares = function(buffer, offset, packet, parent)
  local length = size_of.cancelled_shares
  local range = buffer(offset, length)
  local value = range:uint()
  local display = display.cancelled_shares(value, buffer, offset, packet, parent)
...
size_of.order_cancel_message = function(buffer, offset)
  ...
  index = index + size_of.cancelled_shares
...
dissect.order_cancel_message_fields = function(buffer, offset, packet, parent)
  ...
  index, cancelled_shares = dissect.cancelled_shares(buffer, index, packet, parent)
...
size_of.add_order_mpid_attribution_message = function(buffer, offset)
  local index = 0
...
display.add_order_mpid_attribution_message = function(buffer, offset, size, packet, parent)
  return ""
end

-- Dissect Fields: Add Order Mpid Attribution Message
dissect.add_order_mpid_attribution_message_fields = function(buffer, offset, packet, parent)
  local index = offset
...
dissect.add_order_mpid_attribution_message = function(buffer, offset, packet, parent)
  -- Optionally add struct element to protocol tree
  if show.add_order_mpid_attribution_message then
    local length = size_of.add_order_mpid_attribution_message(buffer, offset)
    local range = buffer(offset, length)
    local display = display.add_order_mpid_attribution_message(buffer, packet, parent)
    parent = parent:add(nasdaq_bx_equities_totalview_itch_v5_0.fields.add_order_mpid_attribution_message, range, display)
  end

  return dissect.add_order_mpid_attribution_message_fields(buffer, offset, packet, parent)
end
...
size_of.payload = function(buffer, offset, message_type)
  ...
  if message_type == "F" then
    return size_of.add_order_mpid_attribution_message(buffer, offset)
  end
...
dissect.payload_branches = function(buffer, offset, packet, parent, message_type)
  ...
  -- Dissect Add Order Mpid Attribution Message
  if message_type == "F" then
    return dissect.add_order_mpid_attribution_message(buffer, offset, packet, parent)
  end
  -- Dissect Order Executed Message
  if message_type == "E" then
