...
nasdaq_nomoptions_bono_itch_v3_2.payload.size = function(buffer, offset, message_type)
  ...
  if message_type == "T" then
    return nasdaq_nomoptions_bono_itch_v3_2.timestamp_message.size(buffer, offset)
  end
  -- Size of System Event Message
  if message_type == "S" then
    return nasdaq_nomoptions_bono_itch_v3_2.system_event_message.size(buffer, offset)
  end
  -- Size of Options Directory Message
  if message_type == "D" then
    return nasdaq_nomoptions_bono_itch_v3_2.options_directory_message.size(buffer, offset)
  end
  -- Size of Trading Action Message
  if message_type == "H" then
    return nasdaq_nomoptions_bono_itch_v3_2.trading_action_message.size(buffer, offset)
  end
  -- Size of Security Open Closed Message
  if message_type == "O" then
    return nasdaq_nomoptions_bono_itch_v3_2.security_open_closed_message.size(buffer, offset)
  end
  -- Size of Short Best Bid And Ask Update Message
  if message_type == "q" then
    return nasdaq_nomoptions_bono_itch_v3_2.short_best_bid_and_ask_update_message.size(buffer, offset)
  end
  -- Size of Long Best Bid And Ask Update Message
  if message_type == "Q" then
    return nasdaq_nomoptions_bono_itch_v3_2.long_best_bid_and_ask_update_message.size(buffer, offset)
  end
  -- Size of Short Best Ask Update Message
  if message_type == "a" then
    return nasdaq_nomoptions_bono_itch_v3_2.short_best_ask_update_message.size(buffer, offset)
  end
  -- Size of Short Best Bid Update Message
  if message_type == "b" then
    return nasdaq_nomoptions_bono_itch_v3_2.short_best_bid_update_message.size(buffer, offset)
  end
  -- Size of Long Best Ask Update Message
  if message_type == "A" then
    return nasdaq_nomoptions_bono_itch_v3_2.long_best_ask_update_message.size(buffer, offset)
  end
  -- Size of Long Best Bid Update Message
  if message_type == "B" then
    return nasdaq_nomoptions_bono_itch_v3_2.long_best_bid_update_message.size(buffer, offset)
  end
  -- Size of Trade Report Message
  if message_type == "R" then
    return nasdaq_nomoptions_bono_itch_v3_2.trade_report_message.size(buffer, offset)
  end
  -- Size of Broken Trade Report Message
  if message_type == "X" then
    return nasdaq_nomoptions_bono_itch_v3_2.broken_trade_report_message.size(buffer, offset)
  end
