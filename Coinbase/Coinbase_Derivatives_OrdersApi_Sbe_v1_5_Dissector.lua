...
-- Display: Data
coinbase_derivatives_ordersapi_sbe_v1_5.data.display = function(buffer, offset, value, packet, parent)
  return ""..value
end
...
coinbase_derivatives_ordersapi_sbe_v1_5.data.dissect = function(buffer, offset, packet, parent)
  if show.data then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_coinbase_derivatives_ordersapi_sbe_v1_5.fields.data, buffer(offset, 0))
    local index = coinbase_derivatives_ordersapi_sbe_v1_5.data.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = coinbase_derivatives_ordersapi_sbe_v1_5.data.display(packet, parent, length)
    parent:append_text(display)
...
coinbase_derivatives_ordersapi_sbe_v1_5.pong_message.dissect = function(buffer, offset, packet, parent)
  if show.pong_message then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_coinbase_derivatives_ordersapi_sbe_v1_5.fields.pong_message, buffer(offset, 0))
    local index = coinbase_derivatives_ordersapi_sbe_v1_5.pong_message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = coinbase_derivatives_ordersapi_sbe_v1_5.pong_message.display(packet, parent, length)
    parent:append_text(display)
...
coinbase_derivatives_ordersapi_sbe_v1_5.ping_message.dissect = function(buffer, offset, packet, parent)
  if show.ping_message then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_coinbase_derivatives_ordersapi_sbe_v1_5.fields.ping_message, buffer(offset, 0))
    local index = coinbase_derivatives_ordersapi_sbe_v1_5.ping_message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = coinbase_derivatives_ordersapi_sbe_v1_5.ping_message.display(packet, parent, length)
    parent:append_text(display)
...
coinbase_derivatives_ordersapi_sbe_v1_5.payload.size = function(buffer, offset, template_id)
  ...
  if template_id == 100 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.logon_message.size
  end
  -- Size of Logon Conf Message
  if template_id == 200 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.logon_conf_message.size
  end
  -- Size of Logout Message
  if template_id == 101 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.logout_message.size
  end
  -- Size of Logged Out Message
  if template_id == 201 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.logged_out_message.size
  end
  -- Size of Heartbeat Message
  if template_id == 10 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.heartbeat_message.size
  end
  -- Size of Test Request Message
  if template_id == 11 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.test_request_message.size
  end
  -- Size of Resend Request Message
  if template_id == 102 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.resend_request_message.size
  end
  -- Size of Gap Fill Message
  if template_id == 202 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.gap_fill_message.size
  end
  ...
  if template_id == 103 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.instrument_info_request_message.size
  end
  -- Size of Instrument Info Message
  if template_id == 203 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.instrument_info_message.size
  end
  -- Size of Set Account Message
  if template_id == 105 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.set_account_message.size
  end
  -- Size of Set Trader Message
  if template_id == 106 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.set_trader_message.size
  end
  -- Size of Set Ack Message
  if template_id == 205 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.set_ack_message.size
  end
  -- Size of New Order Message
  if template_id == 110 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.new_order_message.size
  end
  -- Size of Order Entered Message
  if template_id == 210 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.order_entered_message.size
  end
  -- Size of Replace Order Message
  if template_id == 120 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.replace_order_message.size
  end
  -- Size of Stream Order Message
  if template_id == 121 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.stream_order_message.size
  end
  -- Size of Order Reject Message
  if template_id == 221 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.order_reject_message.size
  end
  -- Size of Order Replaced Message
  if template_id == 220 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.order_replaced_message.size
  end
  -- Size of Cancel Order Message
  if template_id == 130 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.cancel_order_message.size
  end
  -- Size of Order Canceled Message
  if template_id == 230 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.order_canceled_message.size
  end
  -- Size of Cancel Order Reject Message
  if template_id == 233 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.cancel_order_reject_message.size
  end
  -- Size of Mass Cancel Order Message
  if template_id == 131 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.mass_cancel_order_message.size
  end
  -- Size of Mass Cancel Order Ack Message
  if template_id == 231 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.mass_cancel_order_ack_message.size
  end
  -- Size of Mass Cancel Order Reject Message
  if template_id == 232 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.mass_cancel_order_reject_message.size
  end
  -- Size of Unlock Trading Message
  if template_id == 132 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.unlock_trading_message.size
  end
  -- Size of Unlock Trading Ack Message
  if template_id == 234 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.unlock_trading_ack_message.size
  end
  -- Size of Unlock Trading Reject Message
  if template_id == 235 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.unlock_trading_reject_message.size
  end
  -- Size of Order Filled Message
  if template_id == 240 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.order_filled_message.size
  end
  -- Size of Spread Order Filled Message
  if template_id == 241 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.spread_order_filled_message.size
  end
  -- Size of Last Exec Id Request Message
  if template_id == 150 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.last_exec_id_request_message.size
  end
  -- Size of Last Exec Id Message
  if template_id == 250 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.last_exec_id_message.size
  end
  -- Size of Event Resend Request Message
  if template_id == 152 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.event_resend_request_message.size
  end
  -- Size of Event Resend Complete Message
  if template_id == 252 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.event_resend_complete_message.size
  end
  -- Size of Event Resend Reject Message
  if template_id == 253 then
    return coinbase_derivatives_ordersapi_sbe_v1_5.event_resend_reject_message.size
  end
...
local sbe_message_bytes_remaining = function(buffer, index, available)
  ...
  if remaining < coinbase_derivatives_ordersapi_sbe_v1_5.message_header.size then
