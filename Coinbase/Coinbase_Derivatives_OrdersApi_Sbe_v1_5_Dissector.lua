...
coinbase_derivatives_ordersapi_sbe_v1_5.data.dissect = function(buffer, offset, packet, parent)
  ...
  if show.data then
    ...
    local range = buffer(offset, length)
    local value = range:bytes():tohex(false, " ")
    local display = coinbase_derivatives_ordersapi_sbe_v1_5.data.display(buffer, offset, value, packet, parent)
    parent = parent:add(omi_coinbase_derivatives_ordersapi_sbe_v1_5.fields.data, range, value, display)
  end
