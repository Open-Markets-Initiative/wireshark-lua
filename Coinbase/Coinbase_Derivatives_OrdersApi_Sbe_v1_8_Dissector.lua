...
-- Coinbase Derivatives Sbe OrdersApi 1.8 Application Messages
...
-- Display: Flags
coinbase_derivatives_ordersapi_sbe_v1_8.flags.display = function(range, value, packet, parent)
  local flags = {}
...
coinbase_derivatives_ordersapi_sbe_v1_8.flags.bits = function(range, value, packet, parent)

  -- Post Only: 1 Bit
  parent:add(omi_coinbase_derivatives_ordersapi_sbe_v1_8.fields.post_only, range, value)

  -- Reserved 7: 7 Bit
  parent:add(omi_coinbase_derivatives_ordersapi_sbe_v1_8.fields.reserved_7, range, value)
end

-- Dissect: Flags
coinbase_derivatives_ordersapi_sbe_v1_8.flags.dissect = function(buffer, offset, packet, parent)
  local size = coinbase_derivatives_ordersapi_sbe_v1_8.flags.size
  local range = buffer(offset, size)
  local value = range:le_uint()
  local display = coinbase_derivatives_ordersapi_sbe_v1_8.flags.display(range, value, packet, parent)
  local element = parent:add(omi_coinbase_derivatives_ordersapi_sbe_v1_8.fields.flags, range, display)

  if show.flags then
    coinbase_derivatives_ordersapi_sbe_v1_8.flags.bits(range, value, packet, element)
  end
...
-- Display: Data
coinbase_derivatives_ordersapi_sbe_v1_8.data.display = function(buffer, offset, value, packet, parent)
  return ""..value
end
...
coinbase_derivatives_ordersapi_sbe_v1_8.pong_message.dissect = function(buffer, offset, packet, parent)
  if show.pong_message then
    ...
    local length = index - offset
    parent:set_len(length)
    local display = coinbase_derivatives_ordersapi_sbe_v1_8.pong_message.display(packet, parent, length)
    parent:append_text(display)
...
coinbase_derivatives_ordersapi_sbe_v1_8.ping_message.dissect = function(buffer, offset, packet, parent)
  if show.ping_message then
    ...
    local length = index - offset
    parent:set_len(length)
    local display = coinbase_derivatives_ordersapi_sbe_v1_8.ping_message.display(packet, parent, length)
    parent:append_text(display)
...
coinbase_derivatives_ordersapi_sbe_v1_8.payload.size = function(buffer, offset, template_id)
  ...
  if template_id == 100 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.logon_message.size
  end
  -- Size of Logon Conf Message
  if template_id == 200 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.logon_conf_message.size
  end
  -- Size of Logout Message
  if template_id == 101 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.logout_message.size
  end
  -- Size of Logged Out Message
  if template_id == 201 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.logged_out_message.size
  end
  -- Size of Heartbeat Message
  if template_id == 10 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.heartbeat_message.size
  end
  -- Size of Test Request Message
  if template_id == 11 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.test_request_message.size
  end
  -- Size of Resend Request Message
  if template_id == 102 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.resend_request_message.size
  end
  -- Size of Gap Fill Message
  if template_id == 202 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.gap_fill_message.size
  end
  -- Size of Reject Message
  if template_id == 210 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.reject_message.size
  end
  ...
  if template_id == 103 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.instrument_info_request_message.size
  end
  -- Size of Instrument Info Message
  if template_id == 203 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.instrument_info_message.size
  end
  -- Size of Set Account Message
  if template_id == 105 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.set_account_message.size
  end
  -- Size of Set Trader Message
  if template_id == 106 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.set_trader_message.size
  end
  -- Size of Set Ack Message
  if template_id == 205 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.set_ack_message.size
  end
  -- Size of New Order Message
  if template_id == 110 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.new_order_message.size
  end
  -- Size of New Ioc Order Message
  if template_id == 111 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.new_ioc_order_message.size
  end
  -- Size of Order Entered Message
  if template_id == 210 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.order_entered_message.size
  end
  -- Size of Replace Order Message
  if template_id == 120 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.replace_order_message.size
  end
  -- Size of Obsolete Stream Order Message
  if template_id == 121 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.obsolete_stream_order_message.size
  end
  -- Size of Order Reject Message
  if template_id == 221 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.order_reject_message.size
  end
  -- Size of Order Replaced Message
  if template_id == 220 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.order_replaced_message.size
  end
  -- Size of Cancel Order Message
  if template_id == 130 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.cancel_order_message.size
  end
  -- Size of Order Canceled Message
  if template_id == 230 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.order_canceled_message.size
  end
  -- Size of Cancel Order Reject Message
  if template_id == 233 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.cancel_order_reject_message.size
  end
  -- Size of Mass Cancel Order Message
  if template_id == 131 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.mass_cancel_order_message.size
  end
  -- Size of Mass Cancel Order Ack Message
  if template_id == 231 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.mass_cancel_order_ack_message.size
  end
  -- Size of Mass Cancel Order Reject Message
  if template_id == 232 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.mass_cancel_order_reject_message.size
  end
  -- Size of Unlock Trading Message
  if template_id == 132 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.unlock_trading_message.size
  end
  -- Size of Unlock Trading Ack Message
  if template_id == 234 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.unlock_trading_ack_message.size
  end
  -- Size of Unlock Trading Reject Message
  if template_id == 235 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.unlock_trading_reject_message.size
  end
  -- Size of Order Filled Message
  if template_id == 240 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.order_filled_message.size
  end
  -- Size of Spread Order Filled Message
  if template_id == 241 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.spread_order_filled_message.size
  end
  -- Size of Last Exec Id Request Message
  if template_id == 150 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.last_exec_id_request_message.size
  end
  -- Size of Last Exec Id Message
  if template_id == 250 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.last_exec_id_message.size
  end
  -- Size of Event Resend Request Message
  if template_id == 152 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.event_resend_request_message.size
  end
  -- Size of Event Resend Complete Message
  if template_id == 252 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.event_resend_complete_message.size
  end
  -- Size of Event Resend Reject Message
  if template_id == 253 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.event_resend_reject_message.size
  end
  -- Size of Reset Options Fill Protection Message
  if template_id == 160 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.reset_options_fill_protection_message.size
  end
  -- Size of Reset Options Fill Protection Ack Message
  if template_id == 260 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.reset_options_fill_protection_ack_message.size
  end
  -- Size of Reset Options Fill Protection Reject Message
  if template_id == 261 then
    return coinbase_derivatives_ordersapi_sbe_v1_8.reset_options_fill_protection_reject_message.size
  end
...
local sbe_message_bytes_remaining = function(buffer, index, available)
  ...
  if remaining < coinbase_derivatives_ordersapi_sbe_v1_8.message_header.size then
