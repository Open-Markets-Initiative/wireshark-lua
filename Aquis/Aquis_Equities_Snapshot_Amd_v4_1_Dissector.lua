...
omi_aquis_equities_snapshot_amd_v4_1.fields.payload = ProtoField.new("Payload", "aquis.equities.snapshot.amd.v4.1.payload", ftypes.STRING)
...
omi_aquis_equities_snapshot_amd_v4_1.prefs.show_payload = Pref.bool("Show Payload", show.payload, "Parse and add Payload to protocol tree")
...
function omi_aquis_equities_snapshot_amd_v4_1.prefs_changed()
  ...
  if show.payload ~= omi_aquis_equities_snapshot_amd_v4_1.prefs.show_payload then
    show.payload = omi_aquis_equities_snapshot_amd_v4_1.prefs.show_payload
    changed = true
  end
...
-- Size: Payload
aquis_equities_snapshot_amd_v4_1.payload.size = function(buffer, offset, msg_type)
  -- Size of Snapshot Start Message
  if msg_type == 10 then
    return aquis_equities_snapshot_amd_v4_1.snapshot_start_message.size
  end
  -- Size of Book Status Message
  if msg_type == 11 then
    return aquis_equities_snapshot_amd_v4_1.book_status_message.size
  end
  -- Size of Book Entry Message
  if msg_type == 12 then
    return aquis_equities_snapshot_amd_v4_1.book_entry_message.size
  end
  -- Size of Ma C Book Entry Message
  if msg_type == 18 then
    return aquis_equities_snapshot_amd_v4_1.ma_c_book_entry_message.size
  end
...
-- Display: Payload
aquis_equities_snapshot_amd_v4_1.payload.display = function(buffer, offset, packet, parent)
  return ""
end

-- Dissect Branches: Payload
aquis_equities_snapshot_amd_v4_1.payload.branches = function(buffer, offset, packet, parent, msg_type)
  -- Dissect Snapshot Start Message
  if msg_type == 10 then
...
aquis_equities_snapshot_amd_v4_1.payload.dissect = function(buffer, offset, packet, parent, msg_type)
  if not show.payload then
    return aquis_equities_snapshot_amd_v4_1.payload.branches(buffer, offset, packet, parent, msg_type)
  end

  -- Calculate size and check that branch is not empty
  local size = aquis_equities_snapshot_amd_v4_1.payload.size(buffer, offset, msg_type)
  if size == 0 then
    return offset
  end
  ...
  -- Dissect Element
  local range = buffer(offset, size)
  local display = aquis_equities_snapshot_amd_v4_1.payload.display(buffer, packet, parent)
  local element = parent:add(omi_aquis_equities_snapshot_amd_v4_1.fields.payload, range, display)
