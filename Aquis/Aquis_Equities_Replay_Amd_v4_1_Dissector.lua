...
omi_aquis_equities_replay_amd_v4_1.fields.payload = ProtoField.new("Payload", "aquis.equities.replay.amd.v4.1.payload", ftypes.STRING)
...
omi_aquis_equities_replay_amd_v4_1.prefs.show_payload = Pref.bool("Show Payload", show.payload, "Parse and add Payload to protocol tree")
...
function omi_aquis_equities_replay_amd_v4_1.prefs_changed()
  ...
  if show.payload ~= omi_aquis_equities_replay_amd_v4_1.prefs.show_payload then
    show.payload = omi_aquis_equities_replay_amd_v4_1.prefs.show_payload
    changed = true
  end
...
-- Display: Payload
aquis_equities_replay_amd_v4_1.payload.display = function(buffer, offset, packet, parent)
  return ""
end
...
aquis_equities_replay_amd_v4_1.payload.dissect = function(buffer, offset, packet, parent, msg_type)
  ...
  local size = aquis_equities_replay_amd_v4_1.payload.size(buffer, offset, msg_type)
  if size == 0 then
    return offset
  end
  ...
  local display = aquis_equities_replay_amd_v4_1.payload.display(buffer, packet, parent)
  local element = parent:add(omi_aquis_equities_replay_amd_v4_1.fields.payload, range, display)
...
-- Dissector Heuristic for Aquis Equities Replay Amd 4.1
local function omi_aquis_equities_replay_amd_v4_1_heuristic(buffer, packet, parent)
...
omi_aquis_equities_replay_amd_v4_1:register_heuristic("tcp", omi_aquis_equities_replay_amd_v4_1_heuristic)
