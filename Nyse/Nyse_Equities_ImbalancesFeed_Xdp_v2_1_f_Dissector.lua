...
nyse_equities_imbalancesfeed_xdp_v2_1_f.payload.size = function(buffer, offset, message_type)
  ...
  if message_type == 1 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.sequence_number_reset_message.size
  end
  -- Size of Symbol Index Mapping Message
  if message_type == 3 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.symbol_index_mapping_message.size
  end
  -- Size of Retransmission Request Message
  if message_type == 10 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.retransmission_request_message.size
  end
  -- Size of Request Response Message
  if message_type == 11 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.request_response_message.size
  end
  -- Size of Heartbeat Response Message
  if message_type == 12 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.heartbeat_response_message.size
  end
  -- Size of Symbol Index Mapping Request Message
  if message_type == 13 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.symbol_index_mapping_request_message.size
  end
  -- Size of Refresh Request Message
  if message_type == 15 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.refresh_request_message.size
  end
  -- Size of Message Unavailable Message
  if message_type == 31 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.message_unavailable_message.size
  end
  -- Size of Symbol Clear Message
  if message_type == 32 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.symbol_clear_message.size
  end
  -- Size of Security Status Message
  if message_type == 34 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.security_status_message.size
  end
  -- Size of Refresh Header Message
  if message_type == 35 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.refresh_header_message.size
  end
  -- Size of Imbalance Message
  if message_type == 105 then
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.imbalance_message.size
  end
...
-- Dissect: Message
nyse_equities_imbalancesfeed_xdp_v2_1_f.message.dissect = function(buffer, offset, packet, parent, message_index)
  if show.message then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_nyse_equities_imbalancesfeed_xdp_v2_1_f.fields.message, buffer(offset, 0))
    local index = nyse_equities_imbalancesfeed_xdp_v2_1_f.message.fields(buffer, offset, packet, parent, message_index)
    local length = index - offset
    parent:set_len(length)
    local display = nyse_equities_imbalancesfeed_xdp_v2_1_f.message.display(packet, parent, length)
    parent:append_text(display)

    return index, parent
  else
    -- Skip element, add fields directly
    return nyse_equities_imbalancesfeed_xdp_v2_1_f.message.fields(buffer, offset, packet, parent, message_index)
  end
end
