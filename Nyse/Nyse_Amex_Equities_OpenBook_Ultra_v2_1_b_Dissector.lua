...
-- Nyse Amex Equities Ultra OpenBook 2.1.b Application Messages
...
nyse_amex_equities_openbook_ultra_v2_1_b.delta_update_messages.dissect = function(buffer, offset, packet, parent)
  if show.delta_update_messages then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_nyse_amex_equities_openbook_ultra_v2_1_b.fields.delta_update_messages, buffer(offset, 0))
    local index = nyse_amex_equities_openbook_ultra_v2_1_b.delta_update_messages.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = nyse_amex_equities_openbook_ultra_v2_1_b.delta_update_messages.display(packet, parent, length)
    parent:append_text(display)
...
nyse_amex_equities_openbook_ultra_v2_1_b.full_update_messages.dissect = function(buffer, offset, packet, parent)
  if show.full_update_messages then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_nyse_amex_equities_openbook_ultra_v2_1_b.fields.full_update_messages, buffer(offset, 0))
    local index = nyse_amex_equities_openbook_ultra_v2_1_b.full_update_messages.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = nyse_amex_equities_openbook_ultra_v2_1_b.full_update_messages.display(packet, parent, length)
    parent:append_text(display)
