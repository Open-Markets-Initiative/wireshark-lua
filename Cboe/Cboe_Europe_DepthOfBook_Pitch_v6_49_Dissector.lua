...
cboe_europe_depthofbook_pitch_v6_49_dissect.message_fields = function(buffer, offset, packet, parent, size_of_message)
  local index = offset
...
-- Dissect: Message
cboe_europe_depthofbook_pitch_v6_49_dissect.message = function(buffer, offset, packet, parent, size_of_message)
  local index = offset + size_of_message

  -- Optionally add group/struct element to protocol tree
  if show.message then
    local element = parent:add(cboe_europe_depthofbook_pitch_v6_49.fields.message, buffer(offset, 0))
    local current = cboe_europe_depthofbook_pitch_v6_49_dissect.message_fields(buffer, offset, packet, element, size_of_message)
    element:set_len(size_of_message)
    local display = cboe_europe_depthofbook_pitch_v6_49_display.message(buffer, packet, element)
    element:append_text(display)

    return index, element
  else
    -- Skip element, add fields directly
    cboe_europe_depthofbook_pitch_v6_49_dissect.message_fields(buffer, offset, packet, parent, size_of_message)
