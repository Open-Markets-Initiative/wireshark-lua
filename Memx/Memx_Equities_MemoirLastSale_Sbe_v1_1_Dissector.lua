...
memx_equities_memoirlastsale_sbe_v1_1.payload.size = function(buffer, offset, template_id)
  ...
  if template_id == 1 then
    return memx_equities_memoirlastsale_sbe_v1_1.instrument_directory_message.size
  end
  -- Size of Reg Sho Restriction Message
  if template_id == 2 then
    return memx_equities_memoirlastsale_sbe_v1_1.reg_sho_restriction_message.size
  end
  -- Size of Security Trading Status Message
  if template_id == 3 then
    return memx_equities_memoirlastsale_sbe_v1_1.security_trading_status_message.size
  end
  -- Size of Trading Session Status Message
  if template_id == 5 then
    return memx_equities_memoirlastsale_sbe_v1_1.trading_session_status_message.size
  end
  -- Size of Trade Report Message
  if template_id == 10 then
    return memx_equities_memoirlastsale_sbe_v1_1.trade_report_message.size
  end
  -- Size of Trade Cancel Message
  if template_id == 11 then
    return memx_equities_memoirlastsale_sbe_v1_1.trade_cancel_message.size
  end
  -- Size of Trade Correct Message
  if template_id == 12 then
    return memx_equities_memoirlastsale_sbe_v1_1.trade_correct_message.size
  end
...
memx_equities_memoirlastsale_sbe_v1_1.message.dissect = function(buffer, offset, packet, parent)
  if show.message then
    ...
    local index = memx_equities_memoirlastsale_sbe_v1_1.message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = memx_equities_memoirlastsale_sbe_v1_1.message.display(packet, parent, length)
    parent:append_text(display)
