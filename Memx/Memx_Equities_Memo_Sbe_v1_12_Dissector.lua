...
omi_memx_equities_memo_sbe_v1_12.prefs.show_application_messages = Pref.bool("Show Application Messages", show.application_messages, "Parse and add Application Messages to protocol tree")
...
function omi_memx_equities_memo_sbe_v1_12.prefs_changed()
  ...
  if show.application_messages ~= omi_memx_equities_memo_sbe_v1_12.prefs.show_application_messages then
    show.application_messages = omi_memx_equities_memo_sbe_v1_12.prefs.show_application_messages
    changed = true
  end
...
memx_equities_memo_sbe_v1_12.sbe_message.dissect = function(buffer, offset, packet, parent)
  if show.sbe_message then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_memx_equities_memo_sbe_v1_12.fields.sbe_message, buffer(offset, 0))
    local index = memx_equities_memo_sbe_v1_12.sbe_message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = memx_equities_memo_sbe_v1_12.sbe_message.display(packet, parent, length)
    parent:append_text(display)
...
memx_equities_memo_sbe_v1_12.sequenced_message.dissect = function(buffer, offset, packet, parent)
  if show.sequenced_message then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_memx_equities_memo_sbe_v1_12.fields.sequenced_message, buffer(offset, 0))
    local index = memx_equities_memo_sbe_v1_12.sequenced_message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = memx_equities_memo_sbe_v1_12.sequenced_message.display(packet, parent, length)
    parent:append_text(display)
...
memx_equities_memo_sbe_v1_12.unsequenced_message.dissect = function(buffer, offset, packet, parent)
  if show.unsequenced_message then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_memx_equities_memo_sbe_v1_12.fields.unsequenced_message, buffer(offset, 0))
    local index = memx_equities_memo_sbe_v1_12.unsequenced_message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = memx_equities_memo_sbe_v1_12.unsequenced_message.display(packet, parent, length)
    parent:append_text(display)
