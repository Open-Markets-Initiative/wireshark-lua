...
memx_equities_memoirdepthfeed_sbe_v1_3.payload.size = function(buffer, offset, template_id)
  ...
  if template_id == 1 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.instrument_directory_message.size(buffer, offset)
  end
  -- Size of Reg Sho Restriction Message
  if template_id == 2 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.reg_sho_restriction_message.size(buffer, offset)
  end
  -- Size of Security Trading Status Message
  if template_id == 3 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.security_trading_status_message.size(buffer, offset)
  end
  -- Size of Trading Session Status Message
  if template_id == 5 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.trading_session_status_message.size(buffer, offset)
  end
  -- Size of Order Added Message
  if template_id == 10 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.order_added_message.size(buffer, offset)
  end
  -- Size of Order Deleted Message
  if template_id == 11 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.order_deleted_message.size(buffer, offset)
  end
  -- Size of Order Reduced Message
  if template_id == 12 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.order_reduced_message.size(buffer, offset)
  end
  -- Size of Order Executed Message
  if template_id == 13 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.order_executed_message.size(buffer, offset)
  end
  -- Size of Trade Message
  if template_id == 14 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.trade_message.size(buffer, offset)
  end
  -- Size of Broken Trade Message
  if template_id == 15 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.broken_trade_message.size(buffer, offset)
  end
  -- Size of Corrected Trade Message
  if template_id == 16 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.corrected_trade_message.size(buffer, offset)
  end
  -- Size of Clear Book Message
  if template_id == 18 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.clear_book_message.size(buffer, offset)
  end
  -- Size of Snapshot Complete Message
  if template_id == 100 then
    return memx_equities_memoirdepthfeed_sbe_v1_3.snapshot_complete_message.size(buffer, offset)
  end
