...
omi_boats_equities_memoirlastsale_sbe_v1_3.prefs.show_application_messages = Pref.bool("Show Application Messages", show.application_messages, "Parse and add Application Messages to protocol tree")
...
omi_boats_equities_memoirlastsale_sbe_v1_3.prefs.show_message_index = Pref.bool("Show Message Index", show.message_index, "Show generated message index in protocol tree")

-- Handle changed preferences
function omi_boats_equities_memoirlastsale_sbe_v1_3.prefs_changed()

  -- Check if show options have changed
  if show.application_messages ~= omi_boats_equities_memoirlastsale_sbe_v1_3.prefs.show_application_messages then
    show.application_messages = omi_boats_equities_memoirlastsale_sbe_v1_3.prefs.show_application_messages
  end
  ...
  if show.message_index ~= omi_boats_equities_memoirlastsale_sbe_v1_3.prefs.show_message_index then
    show.message_index = omi_boats_equities_memoirlastsale_sbe_v1_3.prefs.show_message_index
  end
end
...
boats_equities_memoirlastsale_sbe_v1_3.sequenced_message.dissect = function(buffer, offset, packet, parent)
  if show.sequenced_message then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_boats_equities_memoirlastsale_sbe_v1_3.fields.sequenced_message, buffer(offset, 0))
    local index = boats_equities_memoirlastsale_sbe_v1_3.sequenced_message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = boats_equities_memoirlastsale_sbe_v1_3.sequenced_message.display(packet, parent, length)
    parent:append_text(display)
