...
jnx_equities_pts_itch_v1_7.payload.size = function(buffer, offset, message_type)
  ...
  if message_type == "T" then
    return jnx_equities_pts_itch_v1_7.timestamp_seconds_message.size(buffer, offset)
  end
  -- Size of System Event Message
  if message_type == "S" then
    return jnx_equities_pts_itch_v1_7.system_event_message.size(buffer, offset)
  end
  -- Size of Price Tick Size Message
  if message_type == "L" then
    return jnx_equities_pts_itch_v1_7.price_tick_size_message.size(buffer, offset)
  end
  -- Size of Orderbook Directory Message
  if message_type == "R" then
    return jnx_equities_pts_itch_v1_7.orderbook_directory_message.size(buffer, offset)
  end
  -- Size of Trading State Message
  if message_type == "H" then
    return jnx_equities_pts_itch_v1_7.trading_state_message.size(buffer, offset)
  end
  -- Size of Short Selling Price Restriction State Message
  if message_type == "Y" then
    return jnx_equities_pts_itch_v1_7.short_selling_price_restriction_state_message.size(buffer, offset)
  end
  -- Size of Order Added Without Attributes Message
  if message_type == "A" then
    return jnx_equities_pts_itch_v1_7.order_added_without_attributes_message.size(buffer, offset)
  end
  -- Size of Order Added With Attributes Message
  if message_type == "F" then
    return jnx_equities_pts_itch_v1_7.order_added_with_attributes_message.size(buffer, offset)
  end
  -- Size of Order Executed Message
  if message_type == "E" then
    return jnx_equities_pts_itch_v1_7.order_executed_message.size(buffer, offset)
  end
  -- Size of Order Deleted Message
  if message_type == "D" then
    return jnx_equities_pts_itch_v1_7.order_deleted_message.size(buffer, offset)
  end
  -- Size of Order Replaced Message
  if message_type == "U" then
    return jnx_equities_pts_itch_v1_7.order_replaced_message.size(buffer, offset)
  end
