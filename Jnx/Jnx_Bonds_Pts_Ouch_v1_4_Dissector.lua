...
jnx_bonds_pts_ouch_v1_4.unsequenced_message.size = function(buffer, offset, unsequenced_message_type)
  ...
  if unsequenced_message_type == "O" then
    return jnx_bonds_pts_ouch_v1_4.enter_order_message.size(buffer, offset)
  end
  -- Size of Replace Order Message
  if unsequenced_message_type == "U" then
    return jnx_bonds_pts_ouch_v1_4.replace_order_message.size(buffer, offset)
  end
  -- Size of Cancel Order Message
  if unsequenced_message_type == "X" then
    return jnx_bonds_pts_ouch_v1_4.cancel_order_message.size(buffer, offset)
  end
...
jnx_bonds_pts_ouch_v1_4.sequenced_message.size = function(buffer, offset, sequenced_message_type)
  ...
  if sequenced_message_type == "S" then
    return jnx_bonds_pts_ouch_v1_4.system_event_message.size(buffer, offset)
  end
  -- Size of Order Accepted Message
  if sequenced_message_type == "A" then
    return jnx_bonds_pts_ouch_v1_4.order_accepted_message.size(buffer, offset)
  end
  -- Size of Order Replaced Message
  if sequenced_message_type == "U" then
    return jnx_bonds_pts_ouch_v1_4.order_replaced_message.size(buffer, offset)
  end
  -- Size of Order Canceled Message
  if sequenced_message_type == "C" then
    return jnx_bonds_pts_ouch_v1_4.order_canceled_message.size(buffer, offset)
  end
  -- Size of Order Aiq Canceled Message
  if sequenced_message_type == "D" then
    return jnx_bonds_pts_ouch_v1_4.order_aiq_canceled_message.size(buffer, offset)
  end
  -- Size of Order Executed With Counter Party Message
  if sequenced_message_type == "e" then
    return jnx_bonds_pts_ouch_v1_4.order_executed_with_counter_party_message.size(buffer, offset)
  end
  -- Size of Order Rejected Message
  if sequenced_message_type == "J" then
    return jnx_bonds_pts_ouch_v1_4.order_rejected_message.size(buffer, offset)
  end
...
jnx_bonds_pts_ouch_v1_4.payload.size = function(buffer, offset, packet_type)
  ...
  if packet_type == "+" then
    return jnx_bonds_pts_ouch_v1_4.debug_packet.size(buffer, offset)
  end
  -- Size of Login Accepted Packet
  if packet_type == "A" then
    return jnx_bonds_pts_ouch_v1_4.login_accepted_packet.size(buffer, offset)
  end
  -- Size of Login Rejected Packet
  if packet_type == "J" then
    return jnx_bonds_pts_ouch_v1_4.login_rejected_packet.size(buffer, offset)
  end
  ...
  if packet_type == "L" then
    return jnx_bonds_pts_ouch_v1_4.login_request_packet.size(buffer, offset)
  end
...
local soup_bin_tcp_packet_bytes_remaining = function(buffer, index, available)
  ...
  if remaining < jnx_bonds_pts_ouch_v1_4.packet_header.size(buffer, index) then
