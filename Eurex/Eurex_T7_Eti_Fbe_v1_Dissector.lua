...
-- Payload
eurex_t7_eti_fbe_v1.payload = {}

-- Display: Payload
eurex_t7_eti_fbe_v1.payload.display = function(value)
  return "Payload: "..value
end

-- Dissect runtime sized field: Payload
eurex_t7_eti_fbe_v1.payload.dissect = function(buffer, offset, packet, parent, size)
  local range = buffer(offset, size)
  local value = range:bytes():tohex(false, " ")
  local display = eurex_t7_eti_fbe_v1.payload.display(value, packet, parent, size)

  parent:add(omi_eurex_t7_eti_fbe_v1.fields.payload, range, value, display)
...
-- Message
eurex_t7_eti_fbe_v1.message = {}

-- Display: Message
eurex_t7_eti_fbe_v1.message.display = function(packet, parent, length)
  return ""
end

-- Dissect Fields: Message
eurex_t7_eti_fbe_v1.message.fields = function(buffer, offset, packet, parent, size_of_message)
  local index = offset
  ...
  -- Payload: 0 Byte
  index, payload = eurex_t7_eti_fbe_v1.payload.dissect(buffer, index, packet, parent, size_of_payload)

  return index
end

-- Dissect: Message
eurex_t7_eti_fbe_v1.message.dissect = function(buffer, offset, packet, parent, size_of_message)
  local index = offset + size_of_message

  -- Optionally add group/struct element to protocol tree
  if show.structs then
    parent = parent:add(omi_eurex_t7_eti_fbe_v1.fields.message, buffer(offset, 0))
    local current = eurex_t7_eti_fbe_v1.message.fields(buffer, offset, packet, parent, size_of_message)
    parent:set_len(size_of_message)
    local display = eurex_t7_eti_fbe_v1.message.display(buffer, packet, parent)
    parent:append_text(display)

    return index, parent
  else
    -- Skip element, add fields directly
    eurex_t7_eti_fbe_v1.message.fields(buffer, offset, packet, parent, size_of_message)
...
-- Packet
eurex_t7_eti_fbe_v1.packet = {}

-- Verify required size of Tcp packet
eurex_t7_eti_fbe_v1.packet.requiredsize = function(buffer)
  return buffer:len() >= eurex_t7_eti_fbe_v1.message_header.size + eurex_t7_eti_fbe_v1.payload.size
end

-- Dissect Packet
eurex_t7_eti_fbe_v1.packet.dissect = function(buffer, packet, parent)
  local index = 0
  ...
  while index < end_of_payload do
    ...
    if available > 0 then
      index = eurex_t7_eti_fbe_v1.message.dissect(buffer, index, packet, parent, size_of_message)
    else
...
function omi_eurex_t7_eti_fbe_v1.dissector(buffer, packet, parent)
  ...
  return eurex_t7_eti_fbe_v1.packet.dissect(buffer, packet, protocol)
end
...
local function omi_eurex_t7_eti_fbe_v1_tcp_heuristic(buffer, packet, parent)
  ...
  if not eurex_t7_eti_fbe_v1.packet.requiredsize(buffer) then return false end
...
-- Register Heuristic for Eurex T7 Eti Fbe 1.
